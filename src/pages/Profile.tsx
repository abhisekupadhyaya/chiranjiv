...
import { useAuth, getWaitlistStats, type WaitlistStats } from "@/auth"
...
const ACHIEVEMENTS = [
  { id: "early-bird", name: "Early Bird", requirement: { type: "rank", value: 1000 } },
  { id: "influencer", name: "Influencer", requirement: { type: "referrals", value: 5 } },
  { id: "ambassador", name: "Ambassador", requirement: { type: "referrals", value: 10 } },
  { id: "champion", name: "Champion", requirement: { type: "referrals", value: 25 } },
]

const APP_BASE_URL = "https://app.chiranjiv.com"

function resolveAppCta(status?: string | null): { label: string; href: string } {
  switch (status) {
    case "onboarding":
      return { label: "Go to onboarding", href: `${APP_BASE_URL}/onboarding` }
    case "onboarded":
      return { label: "Go to app", href: `${APP_BASE_URL}/` }
    case "blocked":
      return { label: "View account status", href: `${APP_BASE_URL}/blocked` }
    default:
      return { label: "Go to app", href: `${APP_BASE_URL}/` }
  }
}
...
const calculateGamificationMetrics = (rank: number, totalUsers: number, referralsCount: number): GamificationMetrics => {
  const safeTotalUsers = Math.max(totalUsers, 1)
  const percentile = (rank / safeTotalUsers) * 100
  const tierInfo = getTierInfo(percentile)
  const estimatedJump = Math.max(5, Math.floor(Math.sqrt(safeTotalUsers / 100)))
  let nextMilestone: { name: string; required: number; current: number } | null = null
    ...
    <div className="flex flex-col items-center justify-center py-8">
      <div className="relative w-64 h-64">
        <svg className="transform -rotate-90 w-full h-full" viewBox="0 0 200 200">
          <circle cx="100" cy="100" r="90" stroke="currentColor" strokeWidth="12" fill="none" className="text-gray-300 dark:text-gray-700" />
          <circle
            cx="100"
            cy="100"
            r="90"
  ...
  return (
    <div
      className={cn(
        ...
        highlight ? "bg-gradient-to-br from-primary/20 via-secondary/10 to-accent/10 border-primary/30 shadow-md" : "bg-muted/20 border-border/30"
      )}
    >
      <div className="flex items-start justify-between mb-3">
        <div className={cn("text-2xl", highlight && "text-primary")}>{icon}</div>
        {trend && (
          <TrendingUp className={cn("w-4 h-4 transition-transform group-hover:scale-125", trend === "up" ? "text-green-500 rotate-0" : "text-red-500 rotate-180")} />
        )}
      </div>
      <div className="space-y-1">
        <div className={cn("text-3xl font-light tracking-tight", highlight && "text-primary")}>{typeof value === "number" ? displayValue.toLocaleString() : value}</div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <Button
              variant="outline"
              size="icon"
              onClick={() => void handleCopy(referralUrl)}
              className={cn("hover:scale-105 transition-all duration-200 h-10 w-10", copied && "bg-green-500 text-white border-green-500 hover:bg-green-600")}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
        {shareButtons.map((button) => (
          <Button
            ...
            className={cn("flex items-center justify-center gap-2 bg-gradient-to-r text-white border-0 hover:scale-105 transition-all duration-200 shadow-lg h-10", button.color)}
          >
            {button.icon}
...
const Profile = () => {
  ...
  useEffect(() => {
    if (!user || user.accountStatus !== "waitlisted") {
      setStats(null)
      setError("")
      return
    }
  ...
  const isWaitlisted = user.accountStatus === "waitlisted"
  const rank = stats?.rank ?? 0
  const totalUsers = stats?.totalUsers ?? 0
  const referralsCount = stats?.referralsCount ?? 0
  const referralCode = stats?.referralCode ?? ""
  const metrics = calculateGamificationMetrics(Math.max(rank, 1), Math.max(totalUsers, 1), referralsCount)
  const appCta = resolveAppCta(user.accountStatus)
  ...
  return (
    <div className="relative w-full min-h-screen overflow-hidden">
      ...
      <div className="fixed bottom-1/4 right-0 w-[50rem] h-[50rem] bg-primary/10 rounded-full blur-[200px] animate-float pointer-events-none z-0" style={{ animationDelay: "2s" }} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <p className="text-base font-mono">{user.email || "N/A"}</p>
                </div>
              </div>
            </div>
          </CardContent>
          ...
          <div className="relative text-left mb-8">
            <p className="text-base sm:text-lg text-muted-foreground font-light leading-relaxed">
              {isWaitlisted
                ...
                : "Your account now has access to the application experience. Continue to the app to proceed."}
            </p>
          </div>
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div className="w-full space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      className={cn("hover:scale-105 transition-all duration-200 h-10 w-10", referralLinkCopied && "bg-green-500 text-white border-green-500 hover:bg-green-600")}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                  <div className="relative">
                    <CircularRankProgress rank={rank} totalUsers={Math.max(totalUsers, 1)} tierColors={metrics.tierColors} percentile={metrics.percentile} />
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <StatCard icon={<Trophy className="w-5 h-5" />} label="Current Rank" value={rank} highlight />
                    <StatCard icon={<Users className="w-5 h-5" />} label="Referrals" value={referralsCount} trend="up" />
                    <StatCard icon={<Users className="w-5 h-5" />} label="On Waitlist" value={totalUsers} />
                    <StatCard icon={<Share2 className="w-5 h-5" />} label="Total Referrals" value={stats.totalReferrals} />
                  </div>
                </div>

                <div className="max-w-2xl mx-auto w-full">
                  <ShareButtonGrid referralUrl={`${window.location.origin}/?ref=${referralCode}`} showReferralLink={false} />
                </div>
              </div>
            ) : (
              <div className="text-center py-10 text-muted-foreground">No waitlist data available.</div>
            )
          ) : (
            <div className="text-center py-8">
              <Button size="lg" onClick={() => window.location.assign(appCta.href)}>
                {appCta.label}
              </Button>
            </div>
          )}
