    ...
    <div className="pt-24 sm:pt-32 pb-16 sm:pb-24">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
        <div className="text-center mb-12 sm:mb-16">
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-foreground mb-4 text-balance">Our Mission</h1>
          <p className="text-lg sm:text-xl text-muted-foreground text-balance">Building the future of genomics, one person at a time</p>
        </div>
        <div className="space-y-12 sm:space-y-16">
          {blogPosts.map((post) => (
            <BlogPost key={post.id} title={post.title} subtitle={post.subtitle} date={post.date} content={post.content} />
          ))}
        </div>
      </div>
    </div>
